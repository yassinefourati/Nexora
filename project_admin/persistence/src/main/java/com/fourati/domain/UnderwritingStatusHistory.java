...
/**
 * Append-only audit trail of {@link UnderwritingCase} status transitions.
 * Rows are written by {@code UnderwritingCaseService} on every status
 * change — never mutated or deleted. Table has no {@code updated_at}
 * column, so this does not extend {@link BaseEntity}.
 */
@Entity
@Table(name = "underwriting_status_history")
@Getter
@Setter
@NoArgsConstructor
public class UnderwritingStatusHistory {
