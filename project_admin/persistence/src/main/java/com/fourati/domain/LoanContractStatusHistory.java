...
/**
 * Append-only audit trail of {@link LoanContract} status transitions. Rows
 * are written by {@code LoanContractService} on every status change — never
 * mutated or deleted. Table has no {@code updated_at} column, so this does
 * not extend {@link BaseEntity}.
 */
@Entity
@Table(name = "loan_contract_status_history")
@Getter
@Setter
@NoArgsConstructor
