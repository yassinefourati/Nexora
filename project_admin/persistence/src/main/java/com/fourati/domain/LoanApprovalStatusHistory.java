...
/**
 * Append-only audit trail of {@link LoanApproval} status transitions. Rows
 * are written by {@code LoanApprovalService} on every status change — never
 * mutated or deleted. Table has no {@code updated_at} column, so this does
 * not extend {@link BaseEntity}.
 */
@Entity
@Table(name = "loan_approval_status_history")
@Getter
@Setter
@NoArgsConstructor
public class LoanApprovalStatusHistory {
